...
A month view of the **dated** posts: every `event` (which carries
...
**Progressive enhancement, same as the map:**

- **No JS** — `renderCalendarHtml()` in `render.mjs` stacks a `<table>` for
  the current month **plus every month an entry touches** (past or future),
  de-duped and in order — never the empty months between. Capped at 24.
- **With JS** — `scripts/calendar.js` (dependency-free, `defer`) hides those
  grids and shows **one month at a time** with `‹ prev / next › / Today`
  controls, building any month grid on demand from a
  `<script type="application/json" id="calendar-events">` payload. Nav is
  clamped to `[earliest entry … latest entry]` (this month always included).
...
the link goes to the post, which links to the event).

`civilParts()` maps a date-only (`2026-10-20`, all-day) or full-timestamp
`start`/`end` to a `{y,m,d}` in `Europe/Madrid`; day maths runs on integer
epoch-day numbers (the same helper logic is mirrored in `calendar.js`).
Styles are `.cal-*` in `style.css` (grid + nav bar + agenda, dark-mode
aware, chips collapse to bars under 480px). `screenshot.mjs` already hides
`.view-tabs` in the social card and doesn't shoot this page.
